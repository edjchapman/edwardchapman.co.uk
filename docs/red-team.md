...
| —   | **robots.txt contract** (edge rewrites the file)   | Live probe: served, sitemap advertised, no global `Disallow: /`, and — since the 2026-07-21 cutover off Cloudflare's managed robots.txt — the served file must match `public/robots.txt` byte-for-byte; any edge rewrite is an incident |
